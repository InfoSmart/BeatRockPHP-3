...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	<?php while($row = fetch_assoc($maps['get'])) { ?>
	<url>
		<loc><?=PATH?>/<?=_c($row['page'])?></loc>
		<?php if(!empty($row['lastmod'])) { ?><lastmod><?=_c($row['lastmod'])?></lastmod><?php } ?>
		<?php if(!empty($row['priority'])) { ?><priority><?=$row['priority']?></priority><?php } ?>
	</url>
	<?php } ?>
</urlset>