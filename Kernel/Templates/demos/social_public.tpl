<div class="wrapper">
<div class="content">
	<h1>Módulo Social: Obtención de información pública.</h1>

	<form action="%PATH_NOW%" method="POST">
		<p>
			<label for="str">Escribe tu nombre de usuario:</label>
			<input type="text" name="username" value="kolesias123" required />
		</p>

		<p>
			<label for="service">Selecciona el servicio:</label>
			<select name="service">
				<option value="twitter">Twitter</option>
				<option value="facebook">Facebook</option>
				<option value="steam">Steam</option>
			</select>
		</p>

		<p>
			<input type="submit" value="Enviar" />
		</p>
	</form>

	<? if(isset($data)) { ?>
	<div class="result">
	<? if($data == false) { ?>
		No se ha podido encontrar el usuario en el servicio.
	<? } else { ?>
		<?_r($data)?>
	<? } ?>
	</div>
	<? } ?>

	<p>
		Asegurate de ver el código fuente del ejemplo para saber como hacerlo :)
	</p>

	<p>
		<a href="%PATH%/demos/">Atras</a>
	</p>
</div>

<style>
input[type='text']
{
	border-radius: 5px;
	padding: 10px 5px;
	width: 500px;
}

.result
{
	border: 2px solid #D8D8D8;
	border-radius: 3px;
	padding: 10px;
}
</style>

<style>
h1
{
	font-family: "Segoe UI Light", Tahoma, Arial;
	font-weight: bold;
}

footer
{
	border-top: 1px solid #D8D8D8;
	color: gray;
	font-size: 12px;
	margin-top: 10px;
	padding: 10px 0;
}
</style>