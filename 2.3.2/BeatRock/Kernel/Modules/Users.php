...
class Users
{
	...
	{
		$q = query("SELECT * FROM {DA}users_services WHERE identification = '$id' AND service = '$service' LIMIT 1");
	...
	// - $service: Servicio predeterminado.
	public static function Verify($id, $password, $service = "")
	{
		$password = Core::Encrypte($password);
		
		$q = query_rows("SELECT null FROM {DA}users WHERE (username = '$id' OR email = '$id') AND password = '$password' AND service = '$service' LIMIT 1");
		return $q > 0 ? true : false;
	}
	...
	public static function CheckSession()
	{
		...
		$sId = FilterText(Core::theSession("login_id"));
		...
		// Estableciendo información extra del servicio usado por el usuario.
	...
	{
		// Si ya hemos iniciado sesión, cancelar.
		if(LOG_IN)
			return false;
			
		// Verificar si la Cookie existe.
		$mS = FilterText(Core::theCookie("session"));
		$ch = self::Only("id", $mS, "cookie_session");
		
		// Al parecer no existe o es inválida.
