...
class Core
{
	...
	// Función - Definir una sesión. (Según se adecua la situación)
	// - $param: Parametro/Nombre.
	// - $value: Valor, si se deja vacio se retornara el valor actual.
	public static function TheCache($param, $value = '')
	{
		...
		{
			if(!$mem)
				return self::theSession($param);
			else
				return Mem::GetM($param);
		}
	}

	// Función - Eliminar una sesión. (Según se adecua la situación)
	// - $param: Parametro/Nombre.
	...
	public static function Smilies($str, $bbcode = false)
	{
		...
		{
			if(!Mem::Ready())
				$data = Io::Read(BIT . "/Emoticons/$i.txt");
			else
			{
				$data = Mem::Get('emoticon_' . $i);

				if($data == false)
				{
					$data = Io::Read(BIT . "/Emoticons/$i.txt");
					Mem::Set('emoticon_' . $i, $data);
				}
			}

			$str = str_replace($e, '<img src="data:;base64,' . $data . '" alt="' . $e . '" title="' . $e . '" />', $str);
		}
	...
	public static function Translate($str, $from = "en", $to = "", $id = C9A399184CB7790D220EF5E812D7BFF636488705)
	{
		...
		$sstr = md5($str);
		$data = !Mem::Ready() ? $_SESSION['translate'][$sstr] : Mem::Get('translate_' . $sstr);
	...
	// Función - Experimental (No recomendada)
	public static function Find($str, $tofind = Array(), $original = "")
	{
		if(!is_string($str))
			return;

		$fstr = str_ireplace("/", "\\/", $str);

		if(is_string($tofind))
			$fstr = str_ireplace($tofind, "([^<]+)", $fstr);
		else if(is_array($tofind))
		{
			foreach($tofind as $f)
				$fstr = str_ireplace($f, "([^<]+)", $fstr);
		}
		...
		echo $fstr, "<br />";
		preg_match_all($fstr, $original, $res);
	...
	// Función - Envio de cabeceras para simular una foto.
	public static function Photo()
	{
		header("Content-type: image/png");
	}
	
	// Función - Obtención del uso de memoria en Bytes por el proceso de Apache. "httpd"
	...
	// Función - Obtención del uso de la carga media del sistema.
