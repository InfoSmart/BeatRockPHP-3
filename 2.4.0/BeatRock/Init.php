...
$back = !Mem::Ready() ? Core::theSession('backup_time') : Mem::GetM('backup_time');
...
if($config['server']['backup'] AND (empty($back) OR time() > $back))
{
	...
	{
		Mem::SetM('backup_config', Io::Read(KERNEL . 'Configuration.php'));
		Mem::SetM('backup_db', MySQL::Backup('', true));
		Mem::SetM('backup_time', Core::Time(30, 2));
	}
}
...
else if(!$config['server']['backup'])
{
	...
	{
		Mem::Delete('backup_config');
		Mem::Delete('backup_db');
		Mem::Delete('backup_time');
	}
}
